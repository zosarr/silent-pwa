...
// Copy my public key
document.getElementById('copyMyKeyRaw').addEventListener('click', async ()=>{
  const key = window.__myRawB64 || '';
  if (!key) return;
  try {
...
document.getElementById('copyMyKeyJwk').addEventListener('click', async ()=>{
  const val = document.getElementById('myPubKey') ? document.getElementById('myPubKey').value : document.getElementById('myPub').value;
  // extract the JSON at the top (before RAW separator)
  const jsonPart = val.split('\n-- RAW')[0];
  await navigator.clipboard.writeText(jsonPart);
});
