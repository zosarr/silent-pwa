  ...
  function connect(){
    ...
    ws.addEventListener('message',async ev=>{
      try{
        ...
        if (msg.type==='image'){ const b64=await e2e.decrypt(msg.iv,msg.ct); const buf=b64ToAb(b64); const blob=new Blob([buf],{type:msg.mime||'image/jpeg'}); addImage(URL.createObjectURL(blob),'peer');
          try{ playBeep(); }catch(_){}
          notifyIncoming({ title:'Nuova foto', body:'Hai ricevuto una foto.' }); return;}
        if (msg.type==='audio'){ const b64=await e2e.decrypt(msg.iv,msg.ct); const buf=b64ToAb(b64); const blob=new Blob([buf],{type:msg.mime||'audio/webm'}); addAudio(URL.createObjectURL(blob),'peer',msg.mime);
          try{ playBeep(); }catch(_){}
          notifyIncoming({ title:'Nuovo audio', body:'Hai ricevuto un messaggio vocale.' }); return;}
