...
const __dbg = (() => {
  ...
  const btnShow = document.getElementById('debug-show');
  const btnToggle = document.getElementById('debug-toggle');
  const btnCopy = document.getElementById('debug-copy');
  function show() { if (overlay) overlay.style.display = 'block'; }
  function hide() { if (overlay) overlay.style.display = 'none'; }
  function log(msg) {
    const ts = new Date().toISOString().split('T')[1].replace('Z','');
    if (pre) pre.textContent += `[${ts}] ${msg}\n`;
  }
  btnShow?.addEventListener('click', () => show());
  btnToggle?.addEventListener('click', () => hide());
...
// Monkey-patch WebSocket to trace connections
(function(){
  const RealWS = window.WebSocket;
  window.WebSocket = function(url, protocols){
    try {
      __dbg.show();
      __dbg.log(`WS connect → ${url}`);
      if (location.protocol === 'https:' && typeof url === 'string' && url.startsWith('ws://')) {
        __dbg.log('⚠️ Mixed content: HTTPS origin → WS non sicuro. Usa wss:// o avvia la PWA su http:// in LAN.');
      }
      const ws = protocols ? new RealWS(url, protocols) : new RealWS(url);
      ws.addEventListener('open', () => __dbg.log('WS open'));
      ws.addEventListener('error', (e) => __dbg.log('WS error'));
      ws.addEventListener('close', (e) => __dbg.log(`WS close code=${e.code} reason=${e.reason||''}`));
      return ws;
    } catch (e) {
      __dbg.log('WS exception: ' + (e?.message||e));
      throw e;
    }
  };
...
__dbg.log('navigator.onLine=' + navigator.onLine);
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (!FP_BOX) return;
  const my = (MY_FP?.textContent || '').trim();
  const peer = (PEER_FP?.textContent || '').trim();
  const hasFp = my.length > 0 && peer.length > 0;

  if (!hasFp && !SESSION_VERIFIED) {
    // Keep box hidden until fingerprints exist or session verified
