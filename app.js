...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // ===== Notifiche & Bip =====
  let beepEnabled = localStorage.getItem('beepEnabled') === '1';
  let unreadCount = 0;
  let audioCtx = null;
  ...
  async function maybeNotify(kind='messaggio') {
    if (document.visibilityState === 'hidden') {
      try {
        if ('Notification' in window) {
          if (Notification.permission === 'granted') {
            new Notification('Nuovo ' + kind, { body: 'Hai ricevuto un ' + kind, icon: './icons/1.png' });
          }
        }
      } catch(_) {}
      try {
  ...
  function clearUnread() {
    ...
    try { document.title = 'Silent'; } catch(_){}
  }
...
// === Fingerprint UI ===
  ...
  function addMsg(text, who = 'peer') {
    ...
    try{ if(who==='peer') maybeNotify('messaggio'); }catch(_{}){}
  ...
  function addImage(url, who='peer'){
    ...
    els.log.appendChild(li);
    els.log.scrollTop = els.log.scrollHeight;
    try{ if(who==='peer') maybeNotify('foto'); }catch(_{}){}
  ...
  function addAudio(url, who='peer', mime='audio/webm'){
    ...
    els.log.appendChild(li);
    els.log.scrollTop = els.log.scrollHeight;
    try{ if(who==='peer') maybeNotify('audio'); }catch(_{}){}
  ...
  // ===== Pulsante Bip =====
  const toggleBeepBtn = document.getElementById('toggleBeepBtn');
  function renderBeepBtn(){
    if (!toggleBeepBtn) return;
    toggleBeepBtn.textContent = 'Bip: ' + (beepEnabled ? 'ON' : 'OFF');
    toggleBeepBtn.setAttribute('aria-pressed', beepEnabled ? 'true' : 'false');
  }
  renderBeepBtn();
  toggleBeepBtn && toggleBeepBtn.addEventListener('click', async ()=>{
    beepEnabled = !beepEnabled;
    localStorage.setItem('beepEnabled', beepEnabled ? '1' : '0');
    // sblocco audio / notifiche su gesto utente
    try {
      ensureAudioCtx();
