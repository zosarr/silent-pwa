...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function manualDisconnect(){
    ...
    e2e = new E2E();
    setConnState(false);
    if (typeof updateFpStatus === 'function') updateFpStatus();
...
// === Fingerprint UI ===
  ...
  function b64ToAb(b64){
    ...
    for (let i=0;i<bin.length;i++) bytes[i]=bin.charCodeAt(0);
    return bytes.buffer;
  }
  ...
  async function notifyIncoming({ title='Silent', body='', data={} }={}){
    try{
      ...
      if (navigator.serviceWorker && navigator.serviceWorker.ready){
        try{
          ...
          await reg.showNotification(title, { body, icon, badge, tag:'incoming', renotify:true, data });
          return true;
        }catch(_){}
      }
      new Notification(title, { body, icon, badge, tag:'incoming', renotify:true, data });
      return true;
    }catch(_){ return false; }
  }
  ...
  notifBtn && notifBtn.addEventListener('click', async () => {
    const ok = await ensureNotificationPermission();
    if (ok) { notifBtn.textContent = 'Notifiche attive'; notifBtn.disabled = true; }
    else { alert('Consenti le notifiche nelle impostazioni del browser.'); }
  });
  ...
  async function subscribeToPush(){
    ...
    let resp = await fetch('/vapid-public-key').catch(()=>null);
    const data = await (resp?.json?.() || Promise.resolve({publicKey:''}));
    const pubKey = data.publicKey || '';
    if (!pubKey) { alert('Chiave VAPID non configurata sul backend'); return false; }
  ...
  // ===== Suono notifiche (WebAudio) =====
  let SOUND_ENABLED = localStorage.getItem('soundEnabled') === '1';
  let audioCtx = null;
  ...
  if (soundBtn){
    const refreshSoundUi = ()=>{ soundBtn.textContent = SOUND_ENABLED ? 'Suono: ON' : 'Suono'; };
    refreshSoundUi();
    soundBtn.addEventListener('click', async () => {
      SOUND_ENABLED = !SOUND_ENABLED;
      localStorage.setItem('soundEnabled', SOUND_ENABLED ? '1' : '0');
      // On first enable, attempt to unlock AudioContext via user gesture
      if (SOUND_ENABLED) {
        try{
          ensureAudioContext();
  ...
  function shouldNotifyNow(){
    // notifica solo quando l'app non è visibile (background / altra tab / bloccata)
    return (typeof document !== 'undefined') && (document.hidden || document.visibilityState !== 'visible');
  }
  ...
  function connect(){
    ...
    ws.addEventListener('message',async ev=>{
      try{
        ...
        if (!e2e.ready) return;
        if (msg.type==='msg'){ const textPeer=await e2e.decrypt(msg.iv,msg.ct); addMsg(textPeer,'peer'); try{ playBeep(); }catch(_){} if (shouldNotifyNow()) notifyIncoming({ title:'Nuovo messaggio', body:(textPeer||'').slice(0,120) }); return;}
        if (msg.type==='image'){ const b64=await e2e.decrypt(msg.iv,msg.ct); if(!b64||b64.length<8){ console.warn('image decrypt empty'); }
          const buf=b64ToAb(b64); if(!buf || buf.byteLength===0){ console.warn('image buffer empty'); }
          const blob=new Blob([buf],{type:msg.mime||'image/jpeg'}); if (!blob || blob.size===0){ console.warn('image blob empty'); return; } addImage(URL.createObjectURL(blob),'peer');
          try{ playBeep(); }catch(_){}
          if (shouldNotifyNow()) notifyIncoming({ title:'Nuova foto', body:'Hai ricevuto una foto.' }); return;}
        if (msg.type==='audio'){ const b64=await e2e.decrypt(msg.iv,msg.ct); if(!b64||b64.length<8){ console.warn('audio decrypt empty'); }
          const buf=b64ToAb(b64); if(!buf || buf.byteLength===0){ console.warn('audio buffer empty'); }
          const blob=new Blob([buf],{type:msg.mime||'audio/webm'}); if (!blob || blob.size===0){ console.warn('audio blob empty'); return; } addAudio(URL.createObjectURL(blob),'peer',msg.mime);
          try{ playBeep(); }catch(_){}
          if (shouldNotifyNow()) notifyIncoming({ title:'Nuovo audio', body:'Hai ricevuto un messaggio vocale.' }); return;}
